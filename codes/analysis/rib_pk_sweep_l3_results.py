#!/usr/bin/env python3
r"""
rib_pk_sweep_l3_results.py  --  L3 (node_004, attempt 1) Results & analysis.
============================================================================
...
intermediate-p/k RANS sweep, and discharges the five L3 binds the L2 review set:
  ...
  B-L3-5 (ANTI-EMPTY) node_004/ contains results.md, figure, json, npz.
...
(rib_pk_sweep_l3_results.npz); leaves the L2 npz byte-identical.  a priori only.
"""
import hashlib
import json
import os
...
def main():
    ...
    np.savez(os.path.join(RESULTS, "rib_pk_sweep_l3_results.npz"), **out)
    ...
    summary = dict(
        node="node_004", level=3, attempt=1,
        bind_B_L3_1=dict(
    ...
    # ---- console report -----------------------------------------------------
    print("=" * 74)
    print("L3 RESULTS  rib p/k sweep (node_004)  Y_IDX=%d  eps*=%.2f" % (Y_IDX, EPS_STAR))
    print("=" * 74)
    ...
    print("\nwrote codes/results/rib_pk_sweep_l3_results.npz")
    print("wrote development/nodes/node_004/{pk_sweep_l3.json,results.md,fig_pk_sweep_l3.{png,pdf}}")
...
def _results_md(summary, pk_rows, bracket, rho_r2, monotone, rho_eps, rho_cov,
    ...
    lines = []
    A = lines.append
    A("# L3 Results & Analysis -- node_004: the d-/k-type roughness bridge\n")
    A("**Level 3 (Results and analysis), attempt 1.** The intermediate-$p/k$ RANS "
    ...
    A("## B-L3-5 (ANTI-EMPTY)\n")
    A("node_004/ contains: `results.md`, `pk_sweep_l3.json`, "
      "`fig_pk_sweep_l3.{png,pdf}`; results NPZ "
      "`codes/results/rib_pk_sweep_l3_results.npz`.\n")
